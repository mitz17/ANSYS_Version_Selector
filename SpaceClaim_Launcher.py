...
def launch_spaceclaim(exe: str, filepath: str | None, workdir: Path):
    cmd = [exe]
    if filepath:
        cmd.append(filepath)
    try:
        ...
        messagebox.showerror("起動エラー", f"SpaceClaim の起動に失敗しました:\n{e}")
...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        # バージョン（ワンクリック）
        vergrp = ttk.Labelframe(frm, text="SpaceClaim バージョン")
        ...
        # 実行
        ttk.Button(frm, text="SpaceClaim を起動", command=self.run).grid(
    ...
    def browse_file(self):
        f = filedialog.askopenfilename(
            title="SpaceClaim 対応ファイルを選択",
            filetypes=[
                ("SpaceClaim files", ["*.scdoc", "*.step", "*.stp", "*.iges", "*.igs"]),
                ("All files", "*.*"),
            ],
        )
        if f:
    ...
    def run(self):
        ...
        launch_spaceclaim(exe, str(p) if p else None, workdir)
        self.destroy()
...
class SettingsDialog(BaseSettingsDialog):
    def __init__(self, parent, data: dict):
        super().__init__(
            ...
            scan_confirm_message="システムをスキャンして SpaceClaim のバージョンを検索しますか？\n既存のパスが上書きされる可能性があります。",
            scan_empty_message="SpaceClaim のインストールが見つかりませんでした。",
